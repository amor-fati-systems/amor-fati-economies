...
| Residence-to-workplace commuting | A historical NSP 2021 matrix is pinned and assessed in the [employment-attachment source review](employment-attachment-source-review.md); no current origin-destination source is pinned. | Do not create a workplace-voivodeship hard control or call the historical matrix a Q4 observation. |
| Person-to-workplace attachment | Neither BAEL nor the reviewed Census tables identify actual workplaces or employers. | No empirical person-to-workplace edge in this target. A later component may create an explicitly synthetic `Person -> Workplace -> Enterprise` model path. |
