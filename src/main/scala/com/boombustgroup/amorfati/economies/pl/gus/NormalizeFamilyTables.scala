...
/** Event-streams NSP family-type or family-children rows to voivodeship totals.
  */
object NormalizeFamilyTables:
  ...
  def main(args: Array[String]): Unit =
    args match
      case Array(input, output, valueColumn) =>
        ...
        val header = s"region\t$valueColumn\tcount"
        val lines  = totals.toVector.sortBy { case ((region, value), _) => (region, value) }.map { case ((region, value), count) => s"$region\t$value\t$count" }
      ...
      case _                                 =>
        Console.err.println("Usage: NormalizeFamilyTables <input.xlsx> <output.tsv> <value-column>")
        sys.exit(2)
