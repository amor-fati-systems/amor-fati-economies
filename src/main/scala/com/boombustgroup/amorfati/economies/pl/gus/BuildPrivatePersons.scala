...
object BuildPrivatePersons:
  ...
  def main(args: Array[String]): Unit =
    args match
      case Array(persons, collective, denominator, output) =>
        ...
        val lines                                               = privateRows.filter(_._4 > 0L).sortBy((region, sex, band, _) => (region, sex, Bands.indexOf(band))).map { case (region, sex, band, count) =>
          s"$region\t$sex\t$band\tprivate_household\t$count"
        }
        val outputPath                                          = Path.of(output)
        Option(outputPath.getParent).foreach(parent => Files.createDirectories(parent))
        Files.writeString(outputPath, (Header +: lines).mkString("\n") + "\n", UTF_8)
      case _                                               =>
  ...
  private def allocate(rows: Vector[(String, String, String, Long)], total: Long): Vector[Long] =
    ...
    val order                = exact
      .zip(floors)
      .zipWithIndex
      .sortBy { case ((value, _), index) => (-(value - value.setScale(0, BigDecimal.RoundingMode.FLOOR)).toDouble, index) }
      .map(_._2)
