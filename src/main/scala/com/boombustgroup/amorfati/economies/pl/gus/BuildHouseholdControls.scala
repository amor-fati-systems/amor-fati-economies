...
object BuildHouseholdControls:
  ...
  def main(args: Array[String]): Unit =
    args match
      case Array(sizePath, compositionPath, personsPath, householdsPath, membershipPath) =>
        ...
        val compositionShares = read(compositionPath)
          .groupBy(_._1)
          .view
          .mapValues { rows =>
        ...
        write(
          householdsPath,
          Header,
          householdRows.sortBy(row => (row._1, row._2, Compositions.indexOf(row._3))).map { case (region, size, composition, count) =>
            s"$region\t$size\t$composition\t$count"
          },
        )
  ...
  private def read(path: String): Vector[(String, String, Long)] =
    Files
      .readAllLines(Path.of(path), UTF_8)
      .asScala
      .drop(1)
      .flatMap { line =>
  ...
  private def rebase(rows: Vector[(String, Int, String, Long)], targetPersons: Map[String, Long]): Vector[(String, Int, String, Long)] =
    rows.groupBy(_._1).toVector.flatMap { case (region, regionalRows) =>
      ...
      val order     = exact.zipWithIndex
        .sortBy { case ((_, size, _, value), index) => (-(size.toDouble * (value - value.setScale(0, BigDecimal.RoundingMode.FLOOR)).toDouble), index) }
        .map(_._2)
      val selected  = order.iterator
        .scanLeft(remainder)((left, index) => if left >= exact(index)._2 then left - exact(index)._2 else left)
        .zip(order)
        .collect { case (before, index) if before > remainder - exact(index)._2 => index }
        .toSet
      val adjusted  = floors.zipWithIndex.map { case ((r, size, composition, count), index) =>
        (r, size, composition, count + (if selected.contains(index) then 1L else 0L))
      }
  ...
  private def write(path: String, header: String, rows: Vector[String]): Unit =
    Option(Path.of(path).getParent).foreach(parent => Files.createDirectories(parent))
