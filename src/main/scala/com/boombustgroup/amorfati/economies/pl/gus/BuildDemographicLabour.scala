  ...
  def main(args: Array[String]): Unit =
    args match
      case Array(personsPath, baelPath, output) =>
        ...
        val rows    = bael
          .flatMap { case (sex, sourceBand, status, target) =>
          ...
          .groupMapReduce(row => (row._1, row._2, row._3))(_._4)(_ + _)
          .toVector
          .map { case ((sex, band, status), count) => (sex, band, status, count) }
          .filter(_._4 > 0L)
          .sortBy { case (sex, band, status, _) => (sex, Bands.indexOf(band), status) }
        Option(Path.of(output).getParent).foreach(parent => Files.createDirectories(parent))
        Files.writeString(
          Path.of(output),
          (Header +: rows.map { case (sex, band, status, count) => s"$sex\t$band\t$status\t$count" }).mkString("\n") + "\n",
          UTF_8,
        )
      case _                                    =>
  ...
  private def allocate(total: Long, available: Vector[(String, Long)]): Vector[(String, Long)] =
    ...
    if denominator == 0L then available.map((band, _) => band -> 0L)
      ...
      val order     = exact.zipWithIndex
        .sortBy { case ((_, value), index) => (-(value - value.setScale(0, BigDecimal.RoundingMode.FLOOR)).toDouble, index) }
        .map(_._2)
