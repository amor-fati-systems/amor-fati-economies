...
object NormalizeBaelLabour:
              ...
              Ages.get(label).toVector.flatMap { band =>
                if sex == "total" then Vector.empty
                  ...
                  Vector(
                    ...
                    case (s, b, status, Some(value)) => s"$s\t$b\t$status\t$value\tthousands"
                    case (_, _, status, None)        => sys.error(s"Missing BAEL $status value for $sex/$band")
                  }
              }
            }
            .toVector
  ...
  private def parseThousands(value: String): Option[Long] =
    value.replace(" ", "").replace(" ", "").replace(",", ".").toDoubleOption.map(value => math.round(value * 1000d))
